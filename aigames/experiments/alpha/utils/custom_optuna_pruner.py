...
class CustomPyTorchLightningPruningCallback(Callback):
    ...
    def on_validation_end(self, trainer: Trainer, pl_module: LightningModule) -> None:
        self.callback_check(trainer, pl_module)

    def on_train_epoch_end(self, trainer: Trainer, pl_module: LightningModule) -> None:
        self.callback_check(trainer, pl_module)

    def callback_check(self, trainer: Trainer, pl_module: LightningModule) -> None:
        ...
        step = getattr(trainer, self.step_variable)
        should_stop = False
        ...
        if trainer.is_global_zero:
            ...
            self._trial.storage.set_trial_system_attr(self._trial._trial_id, _EPOCH_KEY, step)
    ...
    def check_pruned(self) -> None:
        ...
        if intermediate_values is None:
            return
        for step, score in intermediate_values.items():
            self._trial.report(score, step=int(step))
        if is_pruned:
            step = _trial_system_attrs.get(_EPOCH_KEY)
            raise optuna.TrialPruned(f"Trial was pruned at {self.step_variable} {step}.")
