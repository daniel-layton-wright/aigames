...
class AlphaDatasetMultiHyperparameters(AlphaAgentHyperparametersMulti):
    ...
    device: str = 'cpu'  # The device the network is on, and what we should serve the data on
    dataset_device: str = 'cpu'  # The device to store data on
...
class TrajectoryDataset(AlphaDatasetMulti):
    ...
    def advise_incoming_data_size(self, data_size: int):
        self._enforce_max_size(max(self.hyperparams.max_data_size - data_size, 0))

    def _enforce_max_size(self, max_size: int):
        while sum(len(traj.states) for traj in self.trajectories) > max(max_size, 0):
            self.pop()
