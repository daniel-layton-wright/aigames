...
def main():
    ...
    parser.add_argument('--source_study_name', type=str, required=False, default='tictactoe_lightning')
    parser.add_argument('--dest_study', '-d', type=str, required=True)
    parser.add_argument('--dest_study_name', type=str, required=False, default='tictactoe_lightning')
    args, _ = parser.parse_known_args()

    # Load the source study
    import optuna
    source_storage = JournalStorage(JournalFileStorage(args.source_study))
    source_study = optuna.load_study(study_name=args.source_study_name, storage=source_storage)
