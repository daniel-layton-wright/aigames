...
def main():
    ...
    agent = AlphaAgent(game_class, evaluator, [data_sender], use_tqdm=False, n_mcts=200)
    monitor_listener = MultiprocessTrainingListener(monitor_queue, network, sentinel)
    monitor = WandbMonitor(Connect4)
    dataset = BasicAlphaDataset(process_state=False, min_size=2000)
    ...
    for i in range(n_self_play_procs):
        self_play_proc = mp.Process(target=self_play_process,
                                    kwargs=dict(game_class=Connect4, agent=agent, n_games=10, n_game_counter=n_game_counter,
